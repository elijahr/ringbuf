...
def setup(cythonize=True, **kwargs):
    """Drop-in replacement for :func:`setuptools.setup`, adding Cython
    niceties.
    ...
    if op.exists(setup_cfg_file):
        ...
        with open(setup_cfg_file, "r", encoding="utf8") as fp:
            parsed_setup_cfg = parse_setup_cfg(fp, cythonize=cythonize)
        cython_ext_modules = create_cython_ext_modules(parsed_setup_cfg, profile_cython=profile_cython, debug=debug)

        if cythonize:
            try:
                # pylint: disable=import-outside-toplevel
                from Cython.Build import (
                    cythonize as do_cythonize,
                )
            except ImportError:
                pass
            else:
                cython_ext_modules = do_cythonize(cython_ext_modules)
...
def get_cython_compile_time_env(defaults=None):
    ...
    try:
        import __pypy__  # pylint: disable=import-outside-toplevel,unused-import
...
def extract_args(args_str, args):
    ...
    *remaining_args_str* a string containing the remaining arguments.
    ...
    try:
        ...
        raise Exception("args parsing failed") from exc
    extracted_args = {}
...
def _expand_cython_modules(config, cythonize, pkg_config, base_dir):
    ...
    for section in config.sections():
        if section.startswith(MODULE_SECTION_PREFIX):
            ...
            module_dict = _expand_one_cython_module(config, section, cythonize, pkg_config, base_dir)
...
def _expand_one_cython_module(config, section, cythonize, pkg_config, base_dir):
    ...
    module["extra_compile_args"] = _get_config_list(config, section, "extra_compile_args") + pc_extra_compile_args
    module["extra_link_args"] = _get_config_list(config, section, "extra_link_args") + pc_extra_link_args
...
def _run_pkg_config(pkg_names, command, env):
    return subprocess.check_output(["pkg-config", command] + pkg_names, env=env).decode("utf8")
...
def _get_config_list(config, section, option):
    ...
    try:
        ...
        value = ""
    return f"{defaults_value} {value}".split()
...
def _str_to_bool(value):
    ...
    if value in ("0", "off", "false", "no"):
        return False
    raise ValueError(f"invalid boolean string {repr(value)}")
